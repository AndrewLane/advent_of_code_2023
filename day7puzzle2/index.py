...
def parse_line(line):
    hand_and_bid_regex = r"^([23456789TJQKA]{5})\s+(\d+)$"
    matches = re.match(hand_and_bid_regex, line)
    if not matches:
        raise Exception("Invalid line: " + line)

    cards = matches.group(1)
...
def get_hand_type(cards):
    ...
    sorted_cards_without_jokes = "".join(sorted(card for card in cards if card != "J"))

    grouped_cards = [list(g) for k, g in itertools.groupby(sorted_cards_without_jokes)]
    ...
    elif len(grouped_cards) == 2 and any(len(group) == 4 for group in grouped_cards):
        return HandType.FOUR_OF_A_KIND
    elif len(grouped_cards) == 2:
        return HandType.FULL_HOUSE
    elif len(grouped_cards) == 3 and any(len(group) == 3 for group in grouped_cards):
        return HandType.THREE_OF_A_KIND
    elif len(grouped_cards) == 3:
...
def parse_input(input):
    ...
    hands = []
    for line in lines:
        cards, bid = parse_line(line)
        debugprint("Cards: " + str(cards))
        debugprint("Bid: " + str(bid))
        hands.append({"cards": cards, "bid": bid})

    sorted_hands = sorted(hands, key=lambda hand: calculate_hand_value(hand))
