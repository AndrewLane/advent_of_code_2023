...
input = """F.777F-L7-LL7.F77FF7F77--7|7L-F7-7.FF---7F77.FFJ--FL77.|77L.|F7.F7FF7--7FFL-J--F|--F7FF77FL---F7.FF7..FF7JJ-LF7JJF-L7LJ.|.L-F.F.|.F-.FL-7L.F
...
def find_starting_point_and_size_of_land(lines):
    x = -1
    y = -1
    for line_index, line in enumerate(lines):
        position_of_s = line.find("S")
        if position_of_s != -1:
            x = position_of_s
            y = line_index
...
def is_direction_still_within_the_grid(x, y, num_cols, num_rows, direction):
...
def determine_starting_pipe(full_loop):
    ...
    if first_direction == DirectionWeAreHeadedIn.UP:
        if last_direction == DirectionWeAreHeadedIn.UP:
            return "|"
        elif last_direction == DirectionWeAreHeadedIn.LEFT:
            return "L"
        elif last_direction == DirectionWeAreHeadedIn.RIGHT:
            return "J"
    elif first_direction == DirectionWeAreHeadedIn.DOWN:
        if last_direction == DirectionWeAreHeadedIn.DOWN:
            return "|"
        elif last_direction == DirectionWeAreHeadedIn.LEFT:
            return "F"
        elif last_direction == DirectionWeAreHeadedIn.RIGHT:
            return "7"
    elif first_direction == DirectionWeAreHeadedIn.LEFT:
        if last_direction == DirectionWeAreHeadedIn.RIGHT:
            return "-"
        elif last_direction == DirectionWeAreHeadedIn.DOWN:
            return "J"
        elif last_direction == DirectionWeAreHeadedIn.UP:
            return "7"
    elif first_direction == DirectionWeAreHeadedIn.RIGHT:
        if last_direction == DirectionWeAreHeadedIn.LEFT:
            return "-"
        elif last_direction == DirectionWeAreHeadedIn.DOWN:
            return "L"
        elif last_direction == DirectionWeAreHeadedIn.UP:
...
def parse_input():
    ...
    while at_least_one_active_growth:
        ...
        for row, full_row in enumerate(outside_vertices):
            for col, _ in enumerate(full_row):
                # we can't grow if this one isn't reachable in the first place
                ...
                for direction in DirectionWeAreHeadedIn:
                    ...
                    if direction == DirectionWeAreHeadedIn.DOWN:
                        if input_grid[row][col] in ("-", "J", "7"):
                            # blocked
                            continue
                    if direction == DirectionWeAreHeadedIn.UP:
                    ...
                    if direction == DirectionWeAreHeadedIn.RIGHT:
                        if input_grid[row][col] in ("|", "J", "L"):
                            # blocked
                            continue
                    ...
                    # debugprint("Starting at ", col, row, "heading", direction, "we can grow!")
                    next_x, next_y = get_next_pipe_coordinates(col, row, direction)
