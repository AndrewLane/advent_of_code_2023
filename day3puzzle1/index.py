...
def parse_line_for_symbols(line):
    ...
    for match in re.finditer(r"[^\d\.]", line):
        ...
        # print(f"Found {symbol} at {start}")
        indexes_of_symbols.append(start)
...
def parse_input(input):
    ...
    for line in input.splitlines():
        numbers_found_by_line[index] = parse_line_for_numbers(line)
    ...
    for i in range(total_lines):
        for number in numbers_found_by_line[i]:
            # print(f"Processing {number} on line {i}")
            if is_there_an_adjacent_symbol(number, symbols_by_line, total_lines, i):
                total += number["number"]
